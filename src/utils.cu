...
void compute_residual(pdhg_solver_state_t *state, norm_type_t optimality_norm) {
  ...
  if (state->problem_type == LP) {
    ...
    compute_qp_residual_kernel<<<state->num_blocks_primal_dual,
        ...
        state->pdhg_primal_solution, state->constraint_lower_bound,
        state->constraint_upper_bound, state->variable_lower_bound,
        state->variable_upper_bound, state->pdhg_dual_solution,
  ...
  double half_xQx =
      0.5 * compute_xQx(state, state->pdhg_primal_solution,
                        state->quadratic_objective_term->primal_obj_product);
