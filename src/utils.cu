...
__global__ void compute_qp_residual_kernel(
    double *primal_residual, const double *primal_product, const double *primal_obj_product, const double *primal_solution,
    const double *constraint_lower_bound, const double *constraint_upper_bound,
    ...
    const double *objective_vector, const double *constraint_rescaling,
    const double *variable_rescaling, double *dual_obj_contribution,
    const double *const_lb_finite, const double *const_ub_finite, const double step_size,
    int num_constraints, int num_variables)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
void compute_residual(pdhg_solver_state_t *state, norm_type_t optimality_norm)
{
    ...
    }else if (state->problem_type == CONVEX_QP)
    {
        compute_qp_residual_kernel<<<state->num_blocks_primal_dual, THREADS_PER_BLOCK>>>(
            state->primal_residual, state->primal_product, 
            state->quadratic_objective_term->primal_obj_product, state->pdhg_primal_solution,
            state->constraint_lower_bound, state->constraint_upper_bound,
    ...
    double half_xQx = 0.5 * compute_xQx(state, state->pdhg_primal_solution, state->quadratic_objective_term->primal_obj_product);
    ...
    state->primal_objective_value =
        (state->primal_objective_value + half_xQx) / (state->constraint_bound_rescaling *
