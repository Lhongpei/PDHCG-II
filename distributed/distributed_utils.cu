                                                   ...
                                                   double tolerance)
{
    ...
    for (int i = 0; i < max_iterations; ++i)
    {
        ...
        NCCL_CHECK(ncclAllReduce(
            (const void *)dual_product_d, (void *)dual_product_d, n, ncclDouble, ncclSum, grid_context->nccl_col, 0));
        ...
        NCCL_CHECK(ncclAllReduce((const void *)next_eigenvector_d,
                                 (void *)next_eigenvector_d,
                                 m,
                                 ncclDouble,
                                 ncclSum,
                                 grid_context->nccl_row,
                                 0));
...
void initialize_step_size_and_primal_weight_distributed(pdhg_solver_state_t *state, const pdhg_parameters_t *params)
{
    ...
    {
        double max_sv = estimate_maximum_singular_value_distributed(state->sparse_handle,
                                                                    state->blas_handle,
                                                                    state->grid_context,
